...
class Exp_Ailgn_Forecasting(Exp_Basic):
    def __init__(self, args):
        ...
        super(Exp_Ailgn_Forecasting, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader):
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, x_mark, y_mark) in tqdm(enumerate(vali_loader), total=len(vali_loader), desc='EVALUATE', leave=False):
                ...
                # truths.append(batch_y[:,-self.args.model.pred_len:,:].cpu())    
                # preds.append(outputs[:,-self.args.model.pred_len:,:].cpu())
            ...
            # preds = torch.cat(preds, dim=0)   # sample_num × T × N
            
            # val_loss = self.criterion(truths[:, :, -self.args.model.main_dim:], preds[:, :, -self.args.model.main_dim:])
            # self._batch_compute_loss_eval(truths, preds, "vali")
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, x_mark, y_mark) in tqdm(enumerate(test_loader), total=len(test_loader), desc='Test', leave=False):
                ...
                if self.args.time_mark:
                    outputs, _ = self.model(batch_x, x_mark)
                # elif self.args.model.loss.recon_loss:
                #     outputs, recon = self.model(batch_x)
                else:
                    if self.args.rolling:
                        inference_steps = self.args.model.pred_len // self.args.model.output_len
                        dis = self.args.model.pred_len - inference_steps * self.args.model.output_len
                        if dis != 0:
                            inference_steps += 1
                        ...
                        for j in range(inference_steps):
                            if len(pred_y) != 0:
                                batch_x = torch.cat([batch_x[:, self.args.model.output_len:, :], pred_y[-1]], dim=1)

                            output,_ = self.model(batch_x)
                            pred_y.append(output[:, -self.args.model.output_len:, :])

                        pred_y = torch.cat(pred_y, dim=1)
                        if dis != 0:
                            pred_y = pred_y[:, :-dis, :]
                        outputs = pred_y[:, :self.args.model.pred_len, :]
    ...
    def _compute_loss_eval(self, y_true, y_predicted, type):
        y_true = self.inverse_transform(y_true).reshape((-1, self.args.model.pred_len, y_true.shape[-1]))
        y_predicted = self.inverse_transform(y_predicted).reshape((-1, self.args.model.pred_len, y_predicted.shape[-1]))
        # return compute_all_metrics(y_predicted[:, :, -1], y_true[:, :, -1])

        for i in range(self.args.model.main_dim):
            # x, y = y_predicted[:, :, -(i+1)], y_true[:, :, -(i+1)]
            mae, mse, mape = compute_all_metrics(y_predicted[:, :, -(i+1)], y_true[:, :, -(i+1)])
            self._logger.info('Evaluation {:s} {:s}: - mae - {:.4f} - mse - {:.4f} - mape - {:.4f}'
                              .format(type, self.target_name[self.args.model.main_dim-i-1], mae, mse, mape))
