...
class Exp_Ailgn_ROSE(Exp_Basic):
    def __init__(self, args):
        ...
        super(Exp_Ailgn_ROSE, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, epoch_num, save=False):
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, x_mark, y_mark) in enumerate(vali_loader):
                ...
                outputs, xe, xq = self.model(batch_x)
                
                loss_reconstruct = self.criterion(outputs[:, :, -self.args.model.main_dim:].cpu(), batch_y[:, :, -self.args.model.main_dim:].cpu()).detach().cpu().numpy()
    ...
    def train(self):
        ...
        for epoch in range(self.args.train.freeze_epochs):
            ...
            for i, (batch_x, batch_y, x_mark, y_mark) in enumerate(train_loader):
                ...
                loss_reconstruct = self.criterion(outputs[:, :, -self.args.model.main_dim:].cpu(), batch_y[:, :, -self.args.model.main_dim:].cpu())
                loss_embedding = loss_func1(xe[:, :, -self.args.model.main_dim:, :].detach(), 
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, x_mark, y_mark) in enumerate(test_loader):
                ...
                outputs, xe, xq = self.model(batch_x, self.args.model.zero_shot) 
