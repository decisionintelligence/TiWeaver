...
class Model(nn.Module, EncoderAttrs):
    def __init__(self, args):
        ...
        self.var_att_type = args.model.var_att_type
        if self.var_att_type == "mask_cross_att":
            self.var_atten = MaskedVariableCrossAttention(hidden_dim=self.d_model*1, num_heads=args.model.n_heads)
        elif self.var_att_type == "self_att":
            self.var_atten = AttentionLayer(
                        FullAttention(
                            False,
                            args.model.factor,
        ...
        elif self.var_att_type == "mask_self_att":
            self.var_atten = AttentionLayer(
                        FullAttention(
                            True,
                            args.model.factor,
        ...
        elif self.var_att_type == "cross_att":
            self.var_atten = MaskedVariableCrossAttention(hidden_dim=self.d_model*1, num_heads=args.model.n_heads)
    ...
    def get_setting(self, args):
        setting = 'Dynamic_mask_{}_{}_time{}_lr{}_df{}_dm{}_{}_sl{}_pl{}_t{}_catt{}_fft{}_layer{}_patch{}_revin{}'.format(
            args.model.var_att_type,
            args.data.data_path,
            args.model.time,
            args.train.lr,
            args.model.d_ff,
    ...
    def forward(self, x, x_mark, y_mark, x_mask=None, y_mask=None):
        ...
        if self.use_catt:
            ...
            if self.var_att_type == "mask_cross_att":
                ...
                input_emb, _ = self.var_atten(patch_inputs_emb, patch_inputs_emb, patch_inputs_emb, attn_mask=None)
            elif self.var_att_type == "mask_self_att":
                input_emb, _ = self.var_atten(patch_inputs_emb, patch_inputs_emb, patch_inputs_emb, encoder_mask)
            elif self.var_att_type == "cross_att":
                new_mask = torch.ones_like(marge_patch_mask)
                var_attn_out = self.var_atten(marge_patch_inputs_emb, new_mask)

                # 将结果reshape为encoder输入格式
                input_emb = rearrange(var_attn_out, 'b k n d -> (b k) n d')
        else:
            # print("nousecatt")
            input_emb = patch_inputs_emb
