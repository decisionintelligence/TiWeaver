...
def calculate_seasonality_trend(X):
    stl = STL(X, period=7)
    result = stl.fit()
    trend = result.trend
    ...
    trend_strength = max(0, 1 - (np.var(remainder) / np.var(trend + remainder)))
    denominator = np.var(seasonal + remainder)
    if denominator == 0:
        return 0  # 防止除零错误
    seasonality_strength = max(0, 1 - (np.var(remainder) / denominator))
    return trend_strength, seasonality_strength

def calculate_seasonality(X, period):
    # 使用 STL 分解
...
class Model(nn.Module, EncoderAttrs):
    def __init__(self, args):
        ...
        if self.task_name == 'regular':
            ...
            if self.time_embed_type == 'rope':
                # RoPE: Rotary Position Embedding for irregular time series
                self.rope = ContinuousRoPE(self.d_model, max_freq=10000.0, learnable=False)
            else:
    ...
    def forward(self, x, x_mark, y_mark, x_mask=None, y_mask=None, batch=None):
        ...
        if self.task_name == 'regular':
            ...
            if hasattr(self, 'time_embed_type') and self.time_embed_type == 'rope':
                # RoPE mode: use learnable frequency-based embedding for decoder
                # Generate time embedding via sinusoidal projection of timestamps
                freq_bases = self.rope.freq_bases  # [half_dim]
                angles = y_mark * freq_bases.view(1, 1, 1, -1)  # [B, K, L_pred, half_dim]
                te_pred = torch.cat([torch.sin(angles), torch.cos(angles)], dim=-1)  # [B, K, L_pred, D]
            else:
