...
class Model(nn.Module, EncoderAttrs):
    def __init__(self, args):
        ...
        seq_len = self.seq_len_max_irr if self.seq_len_max_irr  else self.seq_len
        pred_len = self.pred_len_max_irr if self.pred_len_max_irr  else self.pred_len
        # self.head_nf = self.d_model * int((self.seq_len_max_irr - self.min_patch_size) / self.stride + 1)
        self.head_nf = self.d_model * int((seq_len - self.min_patch_size) / self.stride + 1)
    ...
    def get_setting(self, args):
        setting = 'Dynamic_PatchTST-{}_{}_{}_lr{}_df{}_dm{}_{}_sl{}_pl{}_t{}_layer{}_patch{}'.format(
            args.random_seed,
            args.data.data_path,
            args.data.freq,
            args.train.lr,
    ...
    def forward(self, x, x_mark, y_mark, x_mask=None, y_mask=None):
        ...
        input_emb = rearrange(marge_patch_inputs_emb, 'b k n d -> (b k) n d')
