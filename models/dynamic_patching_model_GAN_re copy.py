...
class Model(nn.Module, EncoderAttrs):
    def __init__(self, args):
        ...
        self.intra_patch_graph = IntraPatchGATEncoder(in_dim=self.d_model, hidden_dim=self.d_model)
        # self.merge_patch_graph = PatchClusterGAT(in_dim=self.d_model, out_dim=self.d_model, heads=4, min_patch_size=self.min_patch_size)
        
        # self.gat_patch_graph = GATConv(self.d_model, self.d_model, heads=1, concat=False)
        
        # Prediction Head
        if self.task_name == 'irregular':
    ...
    def get_setting(self, args):
        setting = 'Dynamic_GAN{}_{}_{}_lr{}_df{}_dm{}_{}_sl{}_pl{}_t{}_recon{}_fft{}_layer{}_patch{}_revin{}'.format(
            args.model.odefunc.use,
            args.data.data_path,
            args.data.freq,
            args.train.lr,
            args.model.d_ff,
    ...
    def forward(self, x, x_mark=None, y_mark=None, x_mask=None, y_mask=None):
        ...
        while True:
            ...
            sim_mask = attns[:,0,1:] < self.threshold  # [B*K, num_boarders] 判断相邻patch相似度是否小于阈值
        ...
        # marge_patch_inputs_emb = self.intra_patch_graph(patch_inputs)  # [bs*n_vars x num_patch x hidden_dim]
