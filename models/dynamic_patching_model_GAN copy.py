            ...
            if not BOARDERS[:,1::2].any():
                ...
                for b in range(x_patches.shape[0]):
                    ...
                    for i, g in enumerate(bd):
                        ...
                        patch_emb = x_emb_now[cur_id:cur_id+bd[i]].reshape(-1)
                        ...
                        trend_sea.append(f"sample{b},group{i},{trend[0]},{trend[1]}")


                df = pd.DataFrame(trend_sea)
                df.to_csv(f"gap{self.usegap}_{self.seq_len}_trend_sea.csv", index=False)
                break
            else:
                # 合并patches
                # print("Merge!")
