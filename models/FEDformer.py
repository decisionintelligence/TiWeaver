...
class Model(nn.Module):
    ...
    def __init__(self, args, version="fourier", mode_select="random", modes=32):
        """
        ...
        # Decomp
        self.decomp = series_decomp(configs.model.moving_avg)
        ...
        if self.version == "Wavelets":
            encoder_self_att = MultiWaveletTransform(
                ich=configs.model.d_model, L=1, base="legendre"
            )
            decoder_self_att = MultiWaveletTransform(
                ich=configs.model.d_model, L=1, base="legendre"
            )
            decoder_cross_att = MultiWaveletCross(
                in_channels=configs.model.d_model,
                out_channels=configs.model.d_model,
                seq_len_q=self.seq_len // 2 + self.pred_len,
                seq_len_kv=self.seq_len,
                modes=self.modes,
                ich=configs.model.d_model,
                base="legendre",
                activation="tanh",
            )
        else:
            ...
            decoder_cross_att = FourierCrossAttention(
                ...
                modes=self.modes,
                mode_select_method=self.mode_select,
                num_heads=configs.model.n_heads,
            )
        # Encoder
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AutoCorrelationLayer(
                        decoder_self_att, configs.model.d_model, configs.model.n_heads
                    ),
                    AutoCorrelationLayer(
                        decoder_cross_att, configs.model.d_model, configs.model.n_heads
                    ),
                    configs.model.d_model,
                    configs.model.c_out,
                    configs.model.d_ff,
    ...
    def get_setting(self, args):
        setting = 'FEDformer_{}_{}_data{}_lr{}_loss{}_sl{}_pl{}_{}'.format(
            args.data.data_path,
            args.data.freq,
            args.train.lr,
    ...
    def forward(self, x, x_mark=None, y_mark=None, x_mask=None, y_mask=None):
        dec_out = self.forecast(x, None, None, None)
