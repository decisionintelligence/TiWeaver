...
from Data_Provider.dependencies.MIMIC.MIMIC import MIMIC_III, Activity_time_chunk
...
class Data(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        print(f"getting {flag} set of MIMIC_III in tsdm format")
    ...
    def preprocess(self):
        ...
        if self.seq_len + self.pred_len > 96:
            ...
            print(f"{self.seq_len+self.pred_len=} is too large. Expect the value smaller than 96")
            exit(1)

        # 定义缓存文件路径
        cache_dir = os.path.join(self.dataset_root_path, "cache")
        os.makedirs(cache_dir, exist_ok=True)
        train_cache_path = os.path.join(cache_dir, "train_data.pt")
        val_cache_path = os.path.join(cache_dir, "val_data.pt")
        test_cache_path = os.path.join(cache_dir, "test_data.pt")

        # 定义一个辅助函数用于加载或处理并保存数据
        # def load_or_process(cache_path, configs):
        if os.path.exists(train_cache_path):
            print(f"Loading cached dataset from {train_cache_path}")
            train_data = torch.load(train_cache_path)
            print(f"Loading cached dataset from {val_cache_path}")
            val_data = torch.load(val_cache_path)
            print(f"Loading cached dataset from {test_cache_path}")
            ...
            seen_data, test_data = model_selection.train_test_split(task, train_size= 0.9, random_state = 42, shuffle = False)
            ...
            print(f"Processing and caching dataset to {train_cache_path}")
            train_data = Activity_time_chunk(train_data, self.configs)
            torch.save(train_data, train_cache_path)
            
            print(f"Processing and caching dataset to {val_cache_path}")
            val_data = Activity_time_chunk(val_data, self.configs)
            torch.save(train_data, train_cache_path)
            
            print(f"Processing and caching dataset to {test_cache_path}")
            test_data = Activity_time_chunk(test_data, self.configs)
            torch.save(test_data, test_cache_path)
            # return processed

        # train_data = load_or_process(train_cache_path, train_data, self.configs)
        # val_data = load_or_process(val_cache_path, val_data, self.configs)
        # test_data = load_or_process(test_cache_path, test_data, self.configs)
            ...
            for sample in test_all_data:
                ...
                if self.configs.data.collate_fn == "collate_fn_patch":
                    ...
                    for i in range(n_patch):
                        observations = sample["x_mark"] < ((i + 1) * PATCH_LEN / 96)
                    ...
                    for j in range(n_patch_y):
                        observations = sample["y_mark"] < (((n_patch + j + 1) * PATCH_LEN) / 96)
...
def collate_fn_patch(
    ...
    PRED_LEN = configs.model.pred_len
    assert SEQ_LEN % PATCH_LEN == 0, f"seq_len {SEQ_LEN} should be divisible by patch_len {PATCH_LEN}"
    n_patch: int = SEQ_LEN // PATCH_LEN
    n_patch_y: int = math.ceil(configs.model.pred_len / PATCH_LEN)
    ...
    if configs.data.missing_rate > 0:
        ...
        if num_to_mask > 0:
            ...
            # logger.warning(f"Number of observations {num_available} * missing rate {configs.data.missing_rate} = {num_to_mask} observations to be masked. Tips: either observations are too sparse, or --missing_rate is too small. Consider increase --missing_rate.")
            print(f"Number of observations {num_available} * missing rate {configs.data.missing_rate} = {num_to_mask} observations to be masked. Tips: either observations are too sparse, or --missing_rate is too small. Consider increase --missing_rate.")
