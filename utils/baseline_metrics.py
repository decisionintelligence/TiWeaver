...
def masked_smape(preds, labels, null_val=np.nan, mask=None):
    ...
    denominator = (torch.abs(labels) + torch.abs(preds)) / 2 + 1e-8

    # Calculate the SMAPE
    smape_value = torch.abs(preds - labels) / denominator    # 不带百分比

    # Apply the mask
    smape_value = smape_value * mask
    smape_value = torch.where(torch.isnan(smape_value), torch.zeros_like(smape_value), smape_value)

    return torch.mean(smape_value)


def compute_all_metrics(pred, real, null_value =np.nan):
    # pred, real: B x T x N x 1
...
def sudden_changes_mask(labels, null_val = np.nan, threshold_start = 75, threshold_change = 20):
    '''
